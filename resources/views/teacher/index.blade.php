...
<div class="flex flex-col h-screen overflow-y-auto border-b-8 border-teal-500 rounded-b-md w-full">
  ...
  <div class="flex flex-row justify-between bg-white mx-2 my-2 rounded-md">
    ...
    <form action="/teachers/create" method="GET" class="my-auto mx-5">
      @csrf
  ...
  <center class="my-auto">
    <h1 class="text-xl">No Teachers yet.</h1>
  </center>
  @else
  ...
  <table class="content-table bg-gray-100 w-full">
    <thead class="bg-teal-500" style="position: sticky; top: 0; z-index: 1;">  
      <tr class="text-white">
        <th class="text-left pl-8 pr-2 py-2 text-sm">NIP</th>
        <th class="text-left px-2 py-2 text-sm">Nama</th>
        <th class="text-left px-2 py-2 text-sm">Jenis Kelamin</th>
        <th class="text-left px-2 py-2 text-sm">Alamat</th>
        <th class="text-left px-2 py-2 text-sm">Nomor HP</th>
        <th class="text-left px-5 py-2 text-sm">Actions</th>
      </tr>
    </thead>
    <tbody>
        @foreach ($teachers as $teacher)
        <tr>
          <td class="pl-8 pr-2 py-2 text-xs font-bold md:font-semibold md:text-sm">{{ $teacher->id }}</td>
          <td class="px-2 py-2 text-xs font-bold md:font-semibold md:text-sm">{{ $teacher->nama }}</td>
          <td class="px-2 py-2 text-xs font-bold md:font-semibold md:text-sm">{{ $teacher->jenis_kelamin }}</td>
          <td class="px-2 py-2 text-xs font-bold md:font-semibold md:text-sm">{{ $teacher->alamat }}</td>
          <td class="px-2 py-2 text-xs font-bold md:font-semibold md:text-sm">{{ $teacher->nomor_hp }}</td>
          <td class="text-left px-5 py-2">
            <div class="flex flex-row gap-1">
              <a href="/teachers/{{ $teacher->id }}" class="font-bold text-white hover:shadow-sm hover:-translate-y-1 hover:bg-teal-500 transition ease-in text-xs bg-teal-400 rounded-md my-3 px-3 py-1"><i class="fas fa-eye leading-none text-white text-[15px] md:text-lg lg:text-lg xl:text-lg"></i></a>
              <a href="/teachers/{{ $teacher->id }}/edit" class="font-bold text-white hover:shadow-sm hover:-translate-y-1 hover:bg-orange-400 transition ease-in text-xs bg-orange-300 rounded-md my-3 px-3 py-1"><i class="fas fa-pencil leading-none text-white text-[15px] md:text-lg lg:text-lg xl:text-lg"></i></a>
              <form action="/teachers/{{ $teacher->id }}" method="POST" class="my-3" onclick="return confirm('Delete teacher?')">
                @method('delete')
                @csrf
