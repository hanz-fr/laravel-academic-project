...
<div class="flex flex-col h-screen overflow-y-auto border-b-8 border-teal-500 rounded-b-md w-full">
  ...
  <div class="flex flex-row justify-between bg-white mx-2 my-2 rounded-md">
    ...
    <form action="/subjects/create" method="GET" class="my-auto mx-5">
      @csrf
  ...
  <table class="content-table bg-gray-100 w-full h-min">
    <thead class="bg-teal-500" style="position: sticky; top: 0; z-index: 1;">  
      <tr class="text-white">
        ...
        <th class="text-center pr-5 text-sm py-2">Nama Pelajaran</th>
        <th class="text-center px-5 text-sm py-2">Keterangan</th>
    ...
    <tbody>
        ...
        <tr>
          <td class="text-center px-8 py-2 text-xs md:text-sm font-bold md:font-semibold">{{ $subject->id }}</td>
          <td class="text-center pr-5 py-2 text-xs md:text-sm font-bold md:font-semibold">{{ $subject->nama }}</td>
          <td class="text-center px-5 py-2 text-xs md:text-sm font-bold md:font-semibold">{{ $subject->keterangan }}</td>
          <td class="text-center px-5 py-2 text-xs md:text-sm font-bold md:font-semibold">
            <div class="flex flex-row gap-1 justify-center">
              <a href="/subjects/{{ $subject->id }}" class="font-bold text-white hover:shadow-sm hover:-translate-y-1 hover:bg-teal-500 transition ease-in text-xs bg-teal-400 rounded-md my-3 px-3 py-1"><i class="fas fa-eye leading-none text-white text-[15px] md:text-lg lg:text-lg xl:text-lg"></i></a>
              <a href="/subjects/{{ $subject->id }}/edit" class="font-bold text-white hover:shadow-sm hover:-translate-y-1 hover:bg-orange-400 transition ease-in text-xs bg-orange-300 rounded-md my-3 px-3 py-1"><i class="fas fa-pencil leading-none text-white text-[15px] md:text-lg lg:text-lg xl:text-lg"></i></a>
              <form action="/subjects/{{ $subject->id }}" method="POST" class="my-3" onclick="return confirm('Delete subject?')">
                @method('delete')
                @csrf
