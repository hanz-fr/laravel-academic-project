...
<div class="flex flex-col h-screen overflow-y-auto border-b-8 border-teal-500 rounded-b-md w-full">
  ...
  <div class="flex flex-row justify-between bg-white mx-2 my-2 rounded-md">
    ...
    <div class="flex flex-row">
      <form action="/classes/create" method="GET" class="my-auto mx-5">
        @csrf
  ...
  <center class="my-auto">
    <h1 class="text-xl">No Subjects yet.</h1>
  </center>
  @else
  ...
  <table class="content-table bg-gray-100 w-full">
    <thead class="bg-teal-500" style="position: sticky; top: 0; z-index: 1;">  
      <tr class="text-white">
        <th class="text-center py-2 text-sm">Id</th>
        <th class="text-center py-2 text-sm">Kelas</th>
        <th class="text-center py-2 text-sm">Jurusan</th>
        <th class="text-center py-2 text-sm">Guru</th>
        <th class="text-center py-2 text-sm">Status</th>
        <th class="text-center py-2 text-sm">Actions</th>
      </tr>
    </thead>
    <tbody>
        @foreach ($kelas as $class)
        <tr>
          <td class="text-center pl-5 px-2 py-2 font-bold md:font-semibold text-xs md:text-sm">{{ $class->id }}</td>
          <td class="text-center px-2 py-2 font-bold md:font-semibold text-xs md:text-sm">{{ $class->kelas }}</td>
          <td class="text-center px-2 py-2 font-bold md:font-semibold text-xs md:text-sm">{{ $class->jurusan }}</td>
          <td class="text-center px-2 py-2 font-bold md:font-semibold text-xs md:text-sm">{{ $class->teacher->nama }}</td>
          <td class="text-center px-2 py-2 font-bold md:font-semibold text-xs md:text-sm">{{ $class->status }}</td>
          <td class="text-center py-2 font-bold md:font-semibold text-xs md:text-sm">
            <div class="flex flex-row gap-1 justify-center">
              <a href="/classes/{{ $class->id }}" class="font-bold text-white hover:shadow-sm hover:-translate-y-1 hover:bg-teal-500 transition ease-in text-xs bg-teal-400 rounded-md my-3 px-3 py-1"><i class="fas fa-eye leading-none text-white text-[15px] md:text-lg lg:text-lg xl:text-lg"></i></a>
              <a href="/classes/{{ $class->id }}/edit" class="font-bold text-white hover:shadow-sm hover:-translate-y-1 hover:bg-orange-400 transition ease-in text-xs bg-orange-300 rounded-md my-3 px-3 py-1"><i class="fas fa-pencil leading-none text-white text-[15px] md:text-lg lg:text-lg xl:text-lg"></i></a>
              <form action="/classes/{{ $class->id }}" method="POST" class="my-3" onclick="return confirm('Delete class?')">
                @method('delete')
                @csrf
